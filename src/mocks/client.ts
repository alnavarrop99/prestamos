...
const allClients = http.all(
  import.meta.env.VITE_API + '/clientes/list',
  async ({ request }) => {
...
const createClient = http.post(
  import.meta.env.VITE_API + '/clientes/create',
  async ({ request }) => {
    const auth = request.headers.get('Authorization')
    if (!auth || !auth.includes(token)) throw new Error('not auth')
    ...
    clients?.set(clients?.size + 1, {
      ...newClient,
      id: (clients?.get(clients?.size)?.id ?? 0) + 1,
    })

    return HttpResponse.json<TCLIENT_POST>(clients?.get(clients?.size), {
      status: 201,
    })
  }
)

const updateClientById = http.patch(
  import.meta.env.VITE_API + '/clientes/:cliente_id',
...
const getClientById = http.get(
  import.meta.env.VITE_API + '/clientes/by_id/:cliente_id',
  async ({ params, request }) => {
    const auth = request.headers.get('Authorization')
    if (!auth || !auth.includes(token)) throw new Error('not auth')

    const { cliente_id } = params as { cliente_id: string }
    if (!cliente_id || !clients?.has(+cliente_id))
    ...
    if (!userId) throw new Error("not's defined a owner id field")
    ...
    return HttpResponse.json<TCLIENT_GET>({
      ...
      owner: {
        id: client.owner_id,
        rol_id: getRolByName({ rolName: users?.get(userId)?.nombre as TROLES })
          ?.id,
        nombre: getRolByName({ rolName: users?.get(userId)?.nombre as TROLES })
          ?.nombre,
      },
    })
  }
...
const deleteClientById = http.delete(
  import.meta.env.VITE_API + '/clientes/delete/:cliente_id',
  async ({ params, request }) => {
    const auth = request.headers.get('Authorization')
    if (!auth || !auth.includes(token)) throw new Error('not auth')
