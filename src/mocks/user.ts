...
import { users, token, currents as user } from '@/mocks/data'
import { useToken } from '@/lib/context/login'

export let current = user?.get(
  useToken.getState()?.rol?.rolName ?? 'Administrador'
)

const loginUser = http.post(
  ...
  async ({ request }) => {
    ...
    if (
      ...
      throw new Error('username and password are be incorrect')

    if (username === 'admin') current = user.get('Administrador')
    else if (username === 'user') current = user.get('Usuario')
    else if (username === 'cobrador') current = user.get('Cobrador')
...
const listUsers = http.all(
  import.meta.env.VITE_API + '/users/list',
  async ({ request }) => {
...
const updateUserById = http.patch(
  import.meta.env.VITE_API + '/users/:usuario_id',
...
const userById = http.get(
  import.meta.env.VITE_API + '/users/by_id/:id_usuario',
  async ({ params, request }) => {
    const auth = request.headers.get('Authorization')
    if (!auth || !auth.includes(token)) throw new Error('not auth')
...
const currentUser = http.get(
  import.meta.env.VITE_API + '/users/get_current',
  async ({ request }) => {
    const auth = request.headers.get('Authorization')
    if (!auth || !auth.includes(token)) throw new Error('not auth')
