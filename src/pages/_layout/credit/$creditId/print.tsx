...
export function PrintCreditById( { credit: _credit = {} as TCredit }: TPaymentCreditByIdProps ) {
  ...
  const onSubmit: React.FormEventHandler = (ev) => {
    ...
    ) as Record<keyof typeof text.form, string>
    ...
    <DialogContent className="max-w-lg">
      ...
      >
        <Label className='[&>span]:after:content-["_*_"] [&>span]:after:text-red-500'>
          ...
          <Select required name={'options' as keyof typeof text.form}>
            <SelectTrigger className="w-full border border-primary">
              <SelectValue placeholder={text.form.options.placeholder} />
            </SelectTrigger>
            <SelectContent className='[&_*]:cursor-pointer'>
              { text.form.options.items.map( ( item ) => <SelectItem value={item}>{item}</SelectItem> ) }
            </SelectContent>
          </Select>
        </Label>
      </form>
      <DialogFooter className="justify-end">
        ...
        <Button form="new-client-form" type="submit" >
          {text.button.print}
        </Button>
      </DialogFooter>
    </DialogContent>
...
const text = {
  ...
  description: "Seleccione la opcion deseada para la impresion de los pagos rolacionados con el credito",
  button: {
    close: 'Cerrar',
    print: 'Imprimir',
  },
