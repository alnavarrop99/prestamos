...
export function NewCredit() {
  ...
  const onError: ((error: Error, variables: TCREDIT_POST_BODY, context: unknown) => unknown) = () => {
  ...
  const { mutate: createCredit } = useMutation({ ...postCreditOpt,
    onSuccess,
    onError
  })
...
const text = {
  ...
  descriiption: 'Introdusca los datos correctamente para la creacion de un prestamo en la plataforma',
  error: {
